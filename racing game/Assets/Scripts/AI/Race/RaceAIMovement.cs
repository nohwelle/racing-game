...
public class RaceAIMovement : MonoBehaviour
{
    Racer racer;
    ...
    void Update()
    {
        ...
        for (var i = 0; i < RaceGameManager.Instance.allRacers.Count; i++)
        {
            ...
            if (RaceGameManager.Instance.allRacers[i].GetComponent<RacePlayer>() && RaceGameManager.Instance.allRacers[i].GetComponent<Racer>().currentPlacement > GetComponent<Racer>().currentPlacement)
            {
